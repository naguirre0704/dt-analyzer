...
exports.handler = async function (event) {
  ...
  const query = {
    ...
    query: {
      bool: {
        must: [
          { query_string: { query: `log: "POST" AND log: "update_delivery_route_uber" AND log: "${safeOrderCode}"` } },
          ...(from || to ? [{
            range: {
              "@timestamp": {
                ...(from ? { gte: from } : {}),
  ...
  try {
    ...
    if (hits.length === 0) {
      return {
        statusCode: 404,
        headers: securityHeaders,
        body: JSON.stringify({ error: `No se encontraron logs para order_code "${order_code}"` }),
      };
    }
    ...
    console.error("uber-logs.js error:", err.message);
    return {
      statusCode: 500,
      headers: securityHeaders,
